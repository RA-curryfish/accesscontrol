...
int labelProcess( FILE *fh, char *name, char *proc, list *mapping ){
	...
	map *m = (map *)malloc(sizeof(map));
	/* YOUR CODE GOES HERE */
	element *temp1 = get(mapping,name,&matchMapName);
...
int checkAccess( FILE *fh, char *proc, char *file, int op ){
	...
	element *fileElt;
	/* YOUR CODE GOES HERE */
	procElt = get(&system_mapping,proc,&matchMapName);
	fileElt = get(&system_mapping,file,&matchMapName);
	...
	int file_level;
	/* YOUR CODE GOES HERE */
	proc_level = pos(&lattice,proc_map->l,&matchLevel);
	file_level = pos(&lattice,file_map->l,&matchLevel);
...
int checkTrans( FILE *fh, char *proc, char *file, int op, int ttype ){
	...
	for ( i = 0; i < MAX_OPS; i++ ) {
		...
		if ( op & testop ) {
			/* YOUR CODE GOES HERE */
			element *new = get(&trans_mapping,t,&matchTrans);
			if(new!=NULL){
				/* log result */
