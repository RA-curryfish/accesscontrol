...
int addLabelPolicy( FILE *fh, char *name, char *level_char ){
	...
	/* YOUR CODE GOES HERE */
    element *temp1 = get(&lattice,level_char,&matchLevelName);
    /* hint - make mapping node */
	/* YOUR CODE GOES HERE */
    map *temp2 = (map *)malloc(sizeof(map));
    temp2->len = strlen(name);
...
int addTransPolicy( FILE *fh, char *subj_level, char *obj_level, char* new_level, int op, int ttype ){
	...
	/* YOUR CODE GOES HERE */
    element *temp1 = get(&lattice, subj_level,&matchLevelName);
    element *temp2 = get(&lattice, obj_level,&matchLevelName);
    element *temp3 = get(&lattice, new_level,&matchLevelName);
